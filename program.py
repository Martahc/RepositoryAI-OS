...
archivo = open(os.path.join(os.getcwd(), "resultados.txt"), "w")
...
for fichero in directorio.iterdir():
    ...
    # Solicitud a la API de Grobid
    response = grobid_client.process_pdf("processFulltextDocument", str(fichero), generateIDs=False,
                                         consolidate_header=False, consolidate_citations=True,
                                         include_raw_citations=True, include_raw_affiliations=False,
                                         tei_coordinates=False, segment_sentences=False)
    texto = str(response)
    # Verificar si la respuesta es correcta
    if response[1] == 200:
        print("Codigo de respuesta HTTP: ", response[1])
...
if response[1] == 200:
    ...
    plt.savefig("histograma_numero_figuras.jpg")
